...
if query:
    ...
    try:
        ...
        print(f"DEBUG: search_sets returned {len(results)} results")
        ...
        if results:
            ...
            with st.expander("Show full details and price"):
                if st.button("Fetch details for selected set"):
                    print(f"DEBUG: User selected set {selected_set.set_id} for detailed fetch")
                    with st.spinner("Fetching full details..."):
                        try:
                            full_set = lego_api.fetch_set(selected_set.set_id)
                            print(f"DEBUG: fetch_set successful for {selected_set.set_id}")
